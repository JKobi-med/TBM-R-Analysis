...
ggplot(cpgl_sum, aes(x = CpG)) +
  geom_ribbon(aes(ymin = q25, ymax = q75, group = 1), alpha = 0.25) +   # IQR band
  geom_line(aes(y = median, group = 1), linewidth = 1) +                # median
  geom_point(aes(y = median), size = 2) +
  labs(title = "BDNF CpGs — median with IQR",
       x = "CpG", y = "Expression") +
  theme_minimal() +
...
id_deciles <- eda_bdnf_long %>%
  group_by(`ID.BDNF`) %>%
  summarise(id_mean = mean(Expression, na.rm = TRUE), .groups = "drop") %>%
  mutate(decile = ntile(id_mean, 10))

decile_cpgl <- eda_bdnf_long %>%
  inner_join(id_deciles, by = "ID.BDNF") %>%
  group_by(decile, CpG) %>%
  summarise(median = median(Expression, na.rm = TRUE), .groups = "drop")

ggplot(decile_cpgl,
       aes(x = CpG, y = median, group = factor(decile), color = factor(decile))) +
  geom_line(linewidth = 1) +
  geom_point(size = 1.5) +
  labs(title = "BDNF CpGs — ID mean decile profiles",
       x = "CpG", y = "Median expression (within decile)", color = "Decile") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
wide <- eda_bdnf_long %>%
  select(`ID.BDNF`, CpG, Expression) %>%
  pivot_wider(names_from = CpG, values_from = Expression)

set.seed(1)
k <- 8  # choose 6–12 for ~that many lines
km <- kmeans(scale(wide[,-1]), centers = k, nstart = 25)
...
ggplot(centers, aes(CpG, Expression, group = cluster, color = cluster)) +
  ...
  geom_point(size = 1.5) +
  labs(title = "BDNF CpGs — cluster centroid profiles",
