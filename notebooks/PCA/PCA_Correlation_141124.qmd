---
title: "Principal Component Analysis (correlation matrix)"
...
scaling = standardizing each CpG (mean = 0, sd = 1).
PCA is effectively done on the correlation matrix.


## BDNF
```{r}
cpg_bdnf <- bdnf[11:20]
pca_bdnf <- prcomp(na.omit(cpg_bdnf), scale = TRUE) #prcomp() centers by default!
...
shapiro_p <- sapply(pc_scores_bdnf, function(z) {
  n <- length(z)
  if (n >= 3 && n <= 5000) shapiro.test(z)$p.value else NA_real_
})
shapiro_tab <- data.frame(PC = names(shapiro_p),
                          p_value = as.numeric(shapiro_p),
                          BH_adj = p.adjust(shapiro_p, method = "BH"))
print(shapiro_tab)
